...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($_POST["loginname"])) {
        ...
        echo '<script>showAlert("กรุณากรอกชื่อผู้ใช้งาน")</script>';
        exit(); // ออกถ้าไม่ได้รับค่าชื่อผู้ใช้งาน
    }
    
    // เตรียมคำสั่ง SQL ตามเงื่อนไข
    $stmt = $conn->prepare("SELECT * FROM tblogin WHERE $condition");
    ...
    if ($check_user_result->num_rows == 1) {
        ...
        $stored_password = $user_row["loginpassword"];
    
        if (password_verify($_POST["loginpassword"], $stored_password)) {
        ...
        echo '<script>showAlert("ไม่พบชื่อผู้ใช้งาน")</script>';
    }
